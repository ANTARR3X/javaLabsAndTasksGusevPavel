public class task_3 {
    ...
    public static int findZip(String str){
        int s=str.indexOf("zip");  
        return s==-1 ? s : str.indexOf("zip",s+1);}
    ...
    public static boolean same(int[] a1,int[] a2){
        int b1=a1.length,b2=a2.length; 
        for(int i=0;i<a1.length;i++){
            if(a1[i]!=-9999){
                for(int j=i+1;j<a1.length;j++){
                    if(a1[i]==a1[j]){a1[j]=-9999; b1--;}}}} 
                    for(int i=0;i<a2.length;i++){
                        if(a2[i]!=-9999){
                            for(int j=i+1;j<a2.length;j++){
                                if(a2[i]==a2[j]){a2[j]=-9999; b2--;}}}} 
                                return b1==b2;}

    public static boolean isKaprekar(int n){
        int kv=n*n,d=1,d2=1,i;
        for(i=0;kv/d>0;i++){
            d*=10;
            if(i%2==0){d2*=10;}}
            return kv/d2+kv%d2==n;}
    ...
    public static int nextPrime(int n){
        ...
        for(int i=n+1;i<a.length;i++){
            if(a[i]!=-1){return a[i];}}
    return 0;}
    public static boolean rightTriangle(int a,int b,int c){int x=a*a,y=b*b,z=c*c; 
