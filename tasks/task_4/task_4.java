public class task_4 {
    ...
    public static String toSnakeCase(String str){String st="";
    for(int i=0;i<str.length();i++){char c=str.charAt(i);st+=('A'<=c && c<='Z') ? "_"+(char)(c+32) : c;}
    return st;}

    public static String overTime(float[] a){
        return "$"+((Math.min(17,a[1])-a[0])+a[3]*(Math.max(17,a[1])-17))*a[2];}
