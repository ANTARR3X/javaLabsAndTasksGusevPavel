...
public class task_5 {
    ...
    public static boolean canComplete(String a,String b){
        String rex=""; 
        for(int i=0;i<a.length();i++){rex+=a.charAt(i)+".*";}
        return Pattern.compile(rex,Pattern.CASE_INSENSITIVE).matcher(b).find();
    }
    public static int sumDigProd(int a){
        int b=1; 
        while(a>1){b*=a%10; a/=10;} 
        return b>9 ? sumDigProd(b) : b;}

    public static int sumDigProd(int a,int b){ 
        return sumDigProd(a+b);}

    public static int sumDigProd(int[] ar){int a=0; 
        for(int i=0;i<ar.length;i++){a+=ar[i];}  
        return sumDigProd(a);}
